...
export default async function ListPage() {
  const items = await getContentPostsByType('news', 50)
  return (
    <div className="container py-8">
      <Breadcrumbs items={[{ label: 'Home', href: '/' }, { label: 'News', href: '/news' }]} />
      <h1 className="mt-4 text-2xl font-semibold text-slate-900">News</h1>
        ...
        {items.length === 0 ? (
          ...
          items.map((p) => (
            <div key={p.slug} className="card p-4">
              <Link href={`/news/${p.slug}`} className="text-base font-semibold text-slate-900 hover:underline">
                {p.title}
              </Link>
