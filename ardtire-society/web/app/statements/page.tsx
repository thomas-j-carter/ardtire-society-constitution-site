...
export default async function ListPage() {
  const items = await getContentPostsByType('statements', 50)
  return (
    <div className="container py-8">
      <Breadcrumbs items={[{ label: 'Home', href: '/' }, { label: 'Statements', href: '/statements' }]} />
      <h1 className="mt-4 text-2xl font-semibold text-slate-900">Statements</h1>
        ...
        {items.length === 0 ? (
          ...
          items.map((p) => (
            <div key={p.slug} className="card p-4">
              <Link href={`/statements/${p.slug}`} className="text-base font-semibold text-slate-900 hover:underline">
                {p.title}
              </Link>
